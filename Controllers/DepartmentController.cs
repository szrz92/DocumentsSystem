...
{
    [Authorize(Roles = "SuperAdmin,CompanyAdmin,DepartmentHead")]
    public class DepartmentController : Controller
    {
        DocumentManagementContext _context;
        private readonly IHostingEnvironment hostingEnvironment;
        public DepartmentController(DocumentManagementContext context, IHostingEnvironment environment)
        {
            _context = context;
        ...
        public IActionResult Employees(int companyID, int departmentID)
        {
            EmployeeViewModel viewModel = new EmployeeViewModel();
            viewModel.EmployeeDetails = new List<EmployeeDetail>();
            viewModel.EmployeeDetails = (from emp in _context.Users
                                         where (emp.Company_ID == companyID) && (emp.Department_ID == departmentID)
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    var user = new User()
                    {
                        ...
                        Department_ID = (from usr in _context.Users where usr.UserName == User.Identity.Name select usr.Department_ID).SingleOrDefault(),
                    ...
                    return RedirectToAction("Users", "Department");
                }
                catch (Exception)
                {
