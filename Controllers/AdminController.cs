...
namespace DocumentManagement.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public AdminController(DocumentManagementContext context, IHostingEnvironment environment, IHttpContextAccessor _httpcontextaccessor)
        {
            _context = context;
        ...
        public IActionResult UploadIt(IFormFile file, string address)
        {
            if (file != null)
            {
                try
                {
                    ...
                    TempData["Success"] = "File uploaded successfully";
                    return RedirectToAction("Drive", new { path = address });
                }
                catch (Exception)
                {
                    TempData["Failure"] = "File is not uploaded. Please try again or contact system administrator!";
        ...
        public IActionResult Companies()
        {
            ...
            viewModel.CompanyDetails = new List<CompanyDetail>();
            viewModel.CompanyDetails = (from companies in _context.Companies
                                        select new CompanyDetail()
                                        {
                                            ID = companies.Party_ID,
                                            Name = companies.Name,
                                            PhoneNo = companies.PhoneNo,
        ...
        public ActionResult AddCompany(CompanyViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    TempData["Failure"] = "Company is not added successfully.";
                    return RedirectToAction("Companies", "Admin");
                }
            }
            else
            {
                TempData["Failure"] = "Input format not correct. Company not added.";
                return RedirectToAction("Companies", "Admin");
            }
        }
        #endregion
        ...
        public IActionResult Users()
        {
            try
            {
                ...
                viewModel.Companies = from companies in _context.Companies
                                      select new SelectListItem()
                                      {
                                          Value = Convert.ToString(companies.Party_ID),
                                          Text = companies.Name
                                      };
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    var user = new User()
                    {
                        ...
                        Company_ID = PartyIdToCompanyId(viewModel.Company)
                    };
